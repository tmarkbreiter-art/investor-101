...
def check_icon(status):
    return {"ok": "OK", "warn": "WARN", "block": "BLOCK", "warning": "WARN"}.get(status, "WARN")


def get_css():
    return """
:root{--page:#F5F0E8;--card:#fff;--card2:#FAFAF7;--border:#E2DDD4;--border2:#EDE9E0;--ink:#1A1614;--text:#57534E;--muted:#9D9690;--green:#059669;--green-bg:#F0FDF4;--red:#DC2626;--red-bg:#FFF1F2;--orange:#D97706;--orange-bg:#FFFBEB;--blue:#1D4ED8;--blue-bg:#EFF6FF;}
...
.brand{font-size:17px;font-weight:800;}.brand span{color:#059669;}
...
.pi-v{font-size:20px;font-weight:700;margin-bottom:2px;}.pi-c{font-size:11px;font-weight:600;}
...
.sb-v{font-size:20px;font-weight:700;}.sb-s{font-size:11px;color:var(--muted);margin-top:2px;}
.sec-hdr{display:flex;align-items:baseline;justify-content:space-between;margin-bottom:12px;}
.sec-t{font-size:20px;font-weight:700;}.sec-s{font-size:12px;color:var(--muted);}
...
.sc-dollar{font-size:22px;font-weight:700;}.sc-pct{font-size:11px;color:var(--muted);}
...
.sc-body{display:none;}.sc-body.open{display:block;}
...
.sbar-f{height:100%;border-radius:3px;}.sbar-v{width:32px;text-align:right;font-weight:700;}
...
.cli-name{font-weight:700;min-width:110px;}.cli-detail{color:var(--text);}
...
.rr-gain{color:#059669;font-weight:700;}.rr-loss{color:#DC2626;font-weight:700;}
.rr-ratio{font-size:11px;font-weight:800;padding:3px 10px;border-radius:20px;border:1.5px solid #ccc;background:#f5f5f5;}
.rr-explain{font-size:11px;margin-top:6px;padding:6px 8px;border-radius:6px;display:none;}
...
def build_card(p, config):
    ...
    parts.append("<div class='sc-top'><div>")
    ...
    parts.append("<div class='range-lbl'><span>52wk low $" + p2(l52) + "</span><span>" + str(pos52) + "% up</span><span>$" + p2(h52) + " high</span></div>")
    parts.append("<div class='range-track'><div class='range-pin' style='left:calc(" + str(pos52) + "% - 4px)'></div></div></div>")

    # Score bars
    parts.append("<div class='trace'><div class='trace-t'>Measured Signals - Score: " + str(fs.total) + " / 100</div><div class='score-bars'>")
    for label, val, mx in [("Technical", fs.technical, wt), ("Risk", fs.risk, wr), ("Quality", fs.quality, wq), ("Macro", fs.macro, wm), ("Event", fs.event, we)]:
        ...
        parts.append("<div class='sbar'><span class='sbar-l'>" + label + "</span><div class='sbar-t'><div class='sbar-f' style='width:" + str(pct) + "%;background:" + bar_color(val, mx) + "'></div></div><span class='sbar-v'>" + str(val) + "/" + str(mx) + "</span></div>")
    parts.append("</div></div>")

    # Guardrails
    parts.append("<div class='checks'><div class='cl-t'>Guardrail Checks</div>")
    for label, status, detail in [("No-Chase Rule", gr.no_chase_status, gr.no_chase_detail), ("Trend Alignment", gr.trend_status, gr.trend_detail), ("52wk Position", gr.pos52_status, gr.pos52_detail)]:
        parts.append("<div class='cli'><span style='font-weight:700;min-width:30px'>" + check_icon(status) + "</span><span class='cli-name'>" + label + "</span><span class='cli-detail'>" + detail + "</span></div>")
    parts.append("</div>")

    # Levels
    parts.append("<div class='levels'>")
    parts.append("<div class='lbox'><div class='lbox-l'>Buy Range</div><div class='lbox-v' style='color:#059669'>$" + p2(buy_lo) + "-$" + p2(buy_hi) + "</div></div>")
    parts.append("<div class='lbox'><div class='lbox-l'>Stop Loss</div><div class='lbox-v' style='color:#DC2626'>$" + p2(stop) + "</div></div>")
    parts.append("<div class='lbox'><div class='lbox-l'>Target</div><div class='lbox-v' style='color:#1D4ED8'>$" + p2(target) + "</div></div>")
    parts.append("</div>")

    # Allocation + Risk/Reward
    parts.append("<div class='alloc-section' onclick='event.stopPropagation()'>")
    parts.append("<div class='alloc-row'>")
    parts.append("<span class='alloc-lbl'>My $</span>")
    parts.append("<input class='alloc-inp' id='alloc-inp-" + sym + "' type='number' min='0' placeholder='Enter amount'")
    ...
    parts.append("<div class='rr-title'>Risk / Reward — type a dollar amount above to see your numbers</div>")
    parts.append("<div class='rr-row'><span>💰 Potential gain if price hits target ($" + p2(target) + ")</span><span class='rr-gain' id='rr-gain-" + sym + "'>enter amount</span></div>")
    parts.append("<div class='rr-row'><span>🛑 Potential loss if price hits stop ($" + p2(stop) + ")</span><span class='rr-loss' id='rr-loss-" + sym + "'>enter amount</span></div>")
    ...
    parts.append("</div>")
    parts.append("<button class='ask-axiom-btn' onclick='askAxiomAbout(\"" + sym + "\",\"" + verdict + "\"," + str(fs.total) + "," + str(price) + "," + str(stop) + "," + str(target) + ")'>💬 Ask AXIOM — should I invest in " + sym + "?</button>")
    parts.append("</div>")

    # AI Commentary
    parts.append("<div class='sc-ai'>")
    parts.append("<div class='ai-badge'>AI Coach — plain English explanation</div>")
    parts.append("<div class='ai-why'>" + ai_why + "</div>")
    parts.append("<div class='ai-watch'>⚠️ Watch: " + ai_watch + "</div>")
    parts.append("</div>")
...
class DashboardGenerator:
    ...
    def generate(self, portfolio, pulse, budget):
        ...
        p2 = "{:.2f}".format
        spy_price  = "$" + p2(spy.get("price", 0))
        spy_dp     = "{:+.1f}%".format(spy.get("dp", 0))
        qqq_price  = "$" + p2(qqq.get("price", 0))
        qqq_dp     = "{:+.1f}%".format(qqq.get("dp", 0))
        uso_price  = "$" + p2(uso.get("price", 0))
        uso_dp     = "{:+.1f}%".format(uso.get("dp", 0))
        vixy_price = "$" + p2(vixy.get("price", 0))
        ...
        parts.append("<div class='tbs'><div class='tbs-l'>Budget</div><div class='tbs-v'>$<input class='budget-input' id='budgetInput' type='number' value='" + str(int(budget)) + "' oninput='recalcTotals()'/></div></div>")
        parts.append("<div class='tbs'><div class='tbs-l'>Invested</div><div class='tbs-v' id='investedVal'>$0.00</div></div>")
        parts.append("<div class='tbs'><div class='tbs-l'>Remaining Cash</div><div class='tbs-v' id='cashVal'>$" + str(int(budget)) + ".00</div></div>")
        ...
        parts.append("<div class='sb'><div class='sb-l'>Remaining Cash</div><div class='sb-v' id='cashStat'>$" + str(int(budget)) + ".00</div><div class='sb-s'>unallocated</div></div>")
        parts.append("</div>")

        parts.append("<div class='sec-hdr'>")
        parts.append("<div class='sec-t'>Today's Portfolio</div>")
        parts.append("<div class='sec-s'>Click any card to expand. Enter your dollar amount to see risk/reward. Updated " + time_str + "</div>")
        parts.append("</div>")
        parts.append("<div class='grid'>" + cards + "</div>")

        parts.append("<div class='footer'>INVESTOR 101 - AXIOM US - Auto-generated " + date_str + " at " + time_str + " ET<br>For educational purposes only - Not investment advice - All investments carry risk<br>Data: Finnhub / yfinance | Commentary: Groq/Gemini | Decisions: Rules-based Python model</div>")
        parts.append("</div>")
        ...
        parts.append("<div class='chat-msgs' id='chatMsgs'><div class='msg ai'>Hi! I know your full portfolio. Type a dollar amount into any stock card to see your risk and potential reward, or ask me anything!</div></div>")
...
function updateRR(sym, price, stop, target){
    ...
    if(alloc <= 0){
        ...
        gainEl.textContent = 'enter amount';
        lossEl.textContent = 'enter amount';
        ratioEl.textContent = '-';
    ...
    if(explainEl){
        ...
        if(ratio >= 2){
            ratioEl.style.cssText = 'background:#F0FDF4;color:#059669;border-color:#059669;font-size:11px;font-weight:800;padding:3px 10px;border-radius:20px;border:1.5px solid;';
            explainEl.style.cssText = 'display:block;font-size:11px;margin-top:6px;padding:6px 8px;border-radius:6px;background:#F0FDF4;color:#059669;';
            explainEl.textContent = '\u2705 Strong setup \u2014 for every $1 you risk losing, you could gain $' + ratio.toFixed(2) + '. This is a favorable trade.';
        } else if(ratio >= 1){
            ratioEl.style.cssText = 'background:#FFFBEB;color:#D97706;border-color:#D97706;font-size:11px;font-weight:800;padding:3px 10px;border-radius:20px;border:1.5px solid;';
            explainEl.style.cssText = 'display:block;font-size:11px;margin-top:6px;padding:6px 8px;border-radius:6px;background:#FFFBEB;color:#D97706;';
            explainEl.textContent = '\u26a0\ufe0f Fair setup \u2014 for every $1 you risk losing, you could gain $' + ratio.toFixed(2) + '. Proceed with caution and keep your position size small.';
        } else {
            ratioEl.style.cssText = 'background:#FFF1F2;color:#DC2626;border-color:#DC2626;font-size:11px;font-weight:800;padding:3px 10px;border-radius:20px;border:1.5px solid;';
            explainEl.style.cssText = 'display:block;font-size:11px;margin-top:6px;padding:6px 8px;border-radius:6px;background:#FFF1F2;color:#DC2626;';
            explainEl.textContent = '\u274c Poor setup \u2014 you are risking more than you could gain. Consider a smaller amount or wait for a better entry price closer to the buy range.';
        }
    }
    recalcTotals();
}
...
function askAxiomAbout(sym, verdict, score, price, stop, target){
    const alloc = getUserAlloc(sym);
    const budget = parseFloat(document.getElementById('budgetInput').value)||BUDGET_DEFAULT;
    const q = 'I am considering ' + sym + ' which has a ' + verdict + ' signal (score: ' + score + '). '
        + 'Current price is $' + price + ', stop loss is $' + stop + ' (meaning I would sell to cut losses if it drops here), '
        + 'and the target price is $' + target + '. My total budget is $' + budget + '. '
        + (alloc > 0 ? 'I am thinking of putting in $' + alloc + '. Is this a good amount and what is my risk?' : 'How much should I put in and what is my risk?');
...
async function sendMsg(){
    ...
    const context = 'Portfolio: ' + PORTFOLIO.map(p =>
        p.sym + ' ' + p.verdict + ' score=' + p.score + ' price=$' + p.price +
        ' stop=$' + p.stop + ' target=$' + p.target + ' myAlloc=$' + getUserAlloc(p.sym)
    ).join(', ') + '. Market: ' + FEAR + '.';
    try {
        const r = await fetch('/api/chat', {
            ...
            body: JSON.stringify({messages:[
                {role:'system', content:'You are AXIOM Coach, an expert investing teacher for a complete beginner. Use plain simple English. Avoid jargon - if you use a term explain it. Be direct, use real dollar amounts. Never say consult a financial advisor. Context: ' + context},
